...
def convert_grounding_to_cls_scores(logits: Tensor,
                                    positive_maps: List[dict],
                                    ) -> Tensor:
    """Convert logits to class scores."""
    assert len(positive_maps) == logits.shape[0]  # batch size
    # print(f"logits is {logits}")
    # print(f"positive_maps[0] is {positive_maps[0]}")
    # predict时的positive_maps是对应label的set
    # positive_maps[0] is {1: [1], 2: [3], 3: [5], 4: [7], 5: [9], 6: [11], 7: [13, 14], 8: [16, 17, 18, 19, 20], 9: [22], 10: [24]}
    # #print(f"positive_maps[0].shape is {positive_maps[0].shape}")
    # print("why-"*100)
    # import sys
    # sys.exit()
