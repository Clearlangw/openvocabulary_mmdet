...
def run_ner(caption: str) -> Tuple[list, list]:
    ...
    # tokens_positive：包含每个名词短语在文本中的所有位置对（起始位置和结束位置）。
    # noun_phrases：提取出来的名词短语列表，代表文本中与物体检测相关的实体。
    return tokens_positive, noun_phrases


# positive_map 形状（gt数，max实体=256）
# 其中gt数等于tokens_positive数，输入这部分之前，已经将tokens_positive每个元素为[0, 10]即实际char的位置
# 其中正样本数对应于 tokens_positive 列表中每个元素（每个框或实体），而 max_num_entities 则代表 token 化后序列的最大长度。
#  初始时，矩阵中所有元素均设为 0。
#  将字符索引转换为 token 索引
#  对于 tokens_positive 中的每个正样本，它给出了原始文本中与该正样本相关的字符范围（如 (beg, end)）。
#  利用 tokenized.char_to_token 方法，将这些字符级的起始和结束位置转换为 token 级的索引。
#  最终输出positive_map,每个gt对应哪几个token
def create_positive_map(tokenized,
    ...
    # print(positive_map.shape) #[gt,256]
    # import sys
    # sys.exit()
    for j, tok_list in enumerate(tokens_positive):
        for (beg, end) in tok_list:
            try:
                beg_pos = tokenized.char_to_token(beg) # 查询第3个字符(从0开始)属于哪个token
                end_pos = tokenized.char_to_token(end - 1) #这里，char_to_token是将字符级别的索引映射到token级别的索引。
...
def create_positive_map_label_to_token(positive_map: Tensor,
    ...
    # print(plus) #1 默认是代表CLS，这个需要考虑加了coop之后的变动
    # print(positive_map[0])
    # print("?"*100)
    # print(positive_map_label_to_token[0+plus])
    # import sys
    # sys.exit()
